...
function extractWeatherData() {
    global $wpdb;
    $table_current_weather = $wpdb->prefix . "thanhtrwf_current";
    $table_forecast_weather = $wpdb->prefix . "thanhtrwf_forecast";
    $today = $wpdb->get_row( "SELECT * FROM $table_current_weather");
    $forecast = $wpdb->get_results("SELECT * FROM $table_forecast_weather");
...
function init() {
    ...
    $result = $wpdb->get_row("SELECT * FROM $table_current_weather");
    $weatherJson = getWeatherData();
    if($weatherJson != 'request_fail') {
        if($wpdb->get_var("SHOW TABLES LIKE '$table_current_weather'") != $table_current_weather) {
            ...
            $delete = $wpdb->query("TRUNCATE TABLE ".$table_current_weather);
        }
        if($wpdb->get_var("SHOW TABLES LIKE '$table_forecast_weather'") != $table_forecast_weather) {
            ...
            $delete = $wpdb->query("TRUNCATE TABLE ".$table_forecast_weather);
        }
        ...
        $wpdb->insert($table_current_weather, array(
        ...
        for ($i = 1; $i <= 7; $i++) {
            ...
            $wpdb->insert($table_forecast_weather, array(
...
function createMenu() {
	add_menu_page('Tung Plugin', 'Tung Plugin', 'administrator', __FILE__, 'extractWeatherData');
}
